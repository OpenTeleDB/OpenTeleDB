 ...
 *
 * xbtsplitloc.c
 * 	  Choose split point code for xbtree implementation (insertpt).
 *
 *
...
static void _xbt_recsplitloc(FindSplitData *state, OffsetNumber firstoldonright, bool newitemonleft,
	int olddataitemstoleft, Size firstoldonrightsz);
static void _xbt_deltasortsplits(FindSplitData *state, double fillfactormult, bool usemult);
static int _xbt_split_cmp(const void *arg1, const void *arg2);
static bool _xbt_afternewitemoff(FindSplitData *state, OffsetNumber maxoff, int leaffillfactor, bool *usemult);
static bool _xbt_adjacenthtid(ItemPointer lowhtid, ItemPointer highhtid);
static OffsetNumber _xbt_bestsplitloc(FindSplitData *state, int perfectpenalty, bool *newitemonleft,
	FindSplitStrat strategy);
static int _xbt_strategy(FindSplitData *state, SplitPoint *leftpage, SplitPoint *rightpage, FindSplitStrat *strategy);
static void _xbt_interval_edges(const FindSplitData *state, SplitPoint **leftinterval, SplitPoint **rightinterval);
static int _xbt_split_penalty(FindSplitData *state, SplitPoint *split);
static inline IndexTuple _xbt_split_lastleft(FindSplitData *state, SplitPoint *split);
static inline IndexTuple _xbt_split_firstright(FindSplitData *state, SplitPoint *split);
 ...
 */
OffsetNumber
_xbt_findsplitloc(Relation rel,
				 Page page,
				 ...
				 bool *newitemonleft)
{
	...
	leftspace = rightspace = PageGetPageSize(page) - SizeOfPageHeaderData - MAXALIGN(sizeof(XBTPageOpaqueData));
	...
	for (offnum = P_FIRSTDATAKEY(opaque); offnum <= maxoff; offnum = OffsetNumberNext(offnum))
	{
		...
		if (offnum > newitemoff)
			_xbt_recsplitloc(&state, offnum, true, olddataitemstoleft, itemsz);
		else if (offnum < newitemoff)
			_xbt_recsplitloc(&state, offnum, false, olddataitemstoleft, itemsz);
		else
		{
			/*
	...
	if (newitemoff > maxoff)
		_xbt_recsplitloc(&state, newitemoff, false, olddataitemstotal, 0);
	...
	else if (_xbt_afternewitemoff(&state, maxoff, leaffillfactor, &usemult))
	{
		/*
	...
	_xbt_deltasortsplits(&state, fillfactormult, usemult);
	 ...
	 */
	perfectpenalty = _xbt_strategy(&state, &leftpage, &rightpage, &strategy);
	 ...
	 */
	foundfirstright = _xbt_bestsplitloc(&state, perfectpenalty, newitemonleft, strategy);
	pfree(state.splits);
 ...
 */
static void 
_xbt_recsplitloc(FindSplitData *state, 
...
static void _xbt_deltasortsplits(FindSplitData *state, double fillfactormult, bool usemult)
{
	...
	qsort(state->splits, state->nsplits, sizeof(SplitPoint), _xbt_split_cmp);
}

/*
 * qsort-style comparator used by _xbt_deltasortsplits()
 */
static int 
_xbt_split_cmp(const void *arg1, const void *arg2)
{
 ...
 */
static bool 
_xbt_afternewitemoff(FindSplitData *state, OffsetNumber maxoff, int leaffillfactor, bool *usemult)
{
	int16 nkeyatts;
	ItemId itemid;
	IndexTuple tup;
	...
	if (state->newitemoff > maxoff)
	{
		...
		keepnatts = _xbt_keep_natts_fast(state->rel, tup, state->newitem);
	...
	if (!_xbt_adjacenthtid(&tup->t_tid, &state->newitem->t_tid))
		return false;
 ...
 */
static bool 
_xbt_adjacenthtid(ItemPointer lowhtid, ItemPointer highhtid)
{
 ...
 * splitting a leaf page or an internal page.  See XBTreeSplitPenalty() for
 * details.
 *
 ...
 */
static OffsetNumber 
_xbt_bestsplitloc(FindSplitData *state, int perfectpenalty, bool *newitemonleft,
	FindSplitStrat strategy)
{
	...
	for (int i = lowsplit; i < highsplit; i++)
	{
		...
		penalty = _xbt_split_penalty(state, state->splits + i);
 ...
 * passed to XBTreeBestsplitloc() as a final constraint on how far caller is
 * willing to go to avoid appending a heap TID when using the many duplicates
 * strategy (it also saves XBTreeBestsplitloc() useless cycles).
 */
static int 
_xbt_strategy(FindSplitData *state, SplitPoint *leftpage, SplitPoint *rightpage, FindSplitStrat *strategy)
{
	...
	*strategy = SPLIT_DEFAULT;
	 ...
	 */
	_xbt_interval_edges(state, &leftinterval, &rightinterval);
	leftmost = _xbt_split_lastleft(state, leftinterval);
	rightmost = _xbt_split_firstright(state, rightinterval);
	 ...
	 */
	perfectpenalty = _xbt_keep_natts_fast(state->rel, leftmost, rightmost);
	 ...
	 */
	leftmost = _xbt_split_lastleft(state, leftpage);
	rightmost = _xbt_split_firstright(state, rightpage);
	...
	if (perfectpenalty <= indnkeyatts)
	{
		*strategy = SPLIT_MANY_DUPLICATES;
		 ...
		 * before a final decision is made, within XBTreeBestsplitloc().
		 */
		return indnkeyatts;
	/*
	...
	else
	{
		...
		perfectpenalty = _xbt_keep_natts_fast(state->rel, hikey, state->newitem);
		if (perfectpenalty <= indnkeyatts)
			*strategy = SPLIT_SINGLE_VALUE;
		else
		{
 ...
 */
static void 
_xbt_interval_edges(const FindSplitData *state, SplitPoint **leftinterval, SplitPoint **rightinterval)
{
	int highsplit = Min(state->interval, state->nsplits);
 ...
 */
static int 
_xbt_split_penalty(FindSplitData *state, SplitPoint *split)
{
	...
	lastleftuple = _xbt_split_lastleft(state, split);
	firstrighttuple = _xbt_split_firstright(state, split);

	Assert(lastleftuple != firstrighttuple);
	return _xbt_keep_natts_fast(state->rel, lastleftuple, firstrighttuple);
}
...
static inline IndexTuple
 _xbt_split_lastleft(FindSplitData *state, SplitPoint *split)
{
	ItemId itemid;
 ...
 */
static inline IndexTuple 
_xbt_split_firstright(FindSplitData *state, SplitPoint *split)
{
	ItemId itemid;
