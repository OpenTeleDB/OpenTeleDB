/*-------------------------------------------------------------------------
 *
 * waldump.c - decode and display WAL for xstore
 * 
...

		{
			...
			if (r == XLOG_BLCKSZ)
			{
				...
				if (longhdr->std.xlp_magic != XLOG_PAGE_MAGIC)
					pg_fatal(gettext("WAL segment \"%s\" has an incorrect magic number. If it's encrypted, use the -K option to pass encryption credentials"),
								fname);
							   ...
							   ForkNumber matchFork)
{
	...
	for (block_id = 0; block_id <= XLogRecMaxBlockId(record); block_id++)
	{
		...
		if ((matchFork == InvalidForkNumber || matchFork == forknum) &&
			(RelFileLocatorEquals(matchRnode, emptyRelFileLocator) ||
			 RelFileLocatorEquals(matchRnode, rnode)) &&
...
main(int argc, char **argv)
{
	...
	static struct option long_options[] = {
		...
		{"teledb_tde_key_cmd", required_argument, NULL, 'K'},
	...
	while ((option = getopt_long(argc, argv, "bB:e:fF:K:n:p:qr:R:s:t:wx:z",
