...
int od_auth_query(od_client_t *client, char *peer)
{
	...
	*/
	...
	if (status != OD_ROUTER_OK) {
		od_error(&instance->logger, "auth_query", auth_client, NULL,
	...
	/* attach */
	status = od_router_attach(router, auth_client, REPLICA_ROLE_ANY, false);
	if (status != OD_ROUTER_OK) {
		od_error(
	...
	if (server->io.io == NULL) {
		...
		rc = od_backend_connect(server, "auth_query", NULL, auth_client);
		if (rc == NOT_OK_RESPONSE) {
			od_debug(&instance->logger, "auth_query", auth_client,
				 server,
				 "failed to acquire backend connection");
	...
	machine_msg_t *msg;
	msg = od_query_do(server, "auth_query", query, user->value, NULL);
	if (msg == NULL) {
