...
static inline int od_parser_next(od_parser_t *parser, od_token_t *token)
{
	...
	if (is_negative || isdigit(*parser->pos)) {
		...
		while (parser->pos < parser->end && isdigit(*parser->pos)) {
			...
			float_num = (double)token->value.num;
			parser->pos++;
		}

		/* if necessary, deal with the fraction part*/
		if (*parser->pos == '.'){
			parser->pos++;
			double fraction = 0.1;
			while (parser->pos < parser->end && isdigit(*parser->pos)) {
				float_num += (*parser->pos - '0') * fraction;
				fraction *= 0.1;
				parser->pos++;
			}
			is_float = 1;
		...
		if (is_float) {
			token->type = OD_PARSER_FLOAT;
			token->value.float_num = float_num;
		}
		return token->type;
	}
	/* symbols */
