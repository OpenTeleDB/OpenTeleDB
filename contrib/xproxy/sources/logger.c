...
static int od_logger_get_filename(od_logger_t *logger, char *patten, char *path, size_t path_len)
{
	time_t t;
	struct tm *tmp;
	...
	if (strftime(path, path_len, patten, tmp) == 0) {
...
int od_logger_open(od_logger_t *logger, char *pattern)
{
	char path[PATH_MAX];

	if (logger->filename_pattern != pattern) {
		if (logger->filename_pattern)
			free(logger->filename_pattern);
		logger->filename_pattern = strdup(pattern);
	}
	if (od_logger_get_filename(logger, pattern, path, sizeof(path)) == -1)
		return -1;
...
od_logger_format(od_logger_t *logger, od_logger_level_t level, char *file, int line, char *context,
...
static inline void od_logger(void *arg)
{
	...
	for (;;) {
		...
		else if (now > last_ts && (now - last_ts) > 60) {
			char path[PATH_MAX];
			if (od_logger_get_filename(logger, logger->filename_pattern,
									   path, sizeof(path)) == 0) {
				if (pg_strcasecmp(path, logger->filename) != 0)
				{
					od_logger_reopen(logger, logger->filename_pattern);
				}
			}
			last_ts = now;
		}
...
void od_logger_write(od_logger_t *logger, od_logger_level_t level, char *file, int line,
		     ...
		     va_list args)
{
	...
	char output[OD_LOGLINE_MAXLEN] = "\0";
	int len;
	len = od_logger_format(logger, level, file, line, context, client, server, fmt,
				  ...
				  char *string)
{
	...
	len = od_logger_format(logger, level, __FILE_NAME__, __LINE__, context, client, server, string,
			       empty_va_list, output, len + 100); // TODO
