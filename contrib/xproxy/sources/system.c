...
static inline void od_system_server(void *arg)
{
	...
	for (;;) {
		...
		od_log(&instance->logger,"system_server", client, NULL, "client accepted, after add, clients_routing: %u, msg: %p", od_atomic_u32_of(&router->clients_routing), msg);

		if (od_worker_pool_feed(worker_pool, msg) == -1) {
			od_error(&instance->logger,"system_server", client, NULL, "od_worker_pool_feed failed, clients_routing: %u", od_atomic_u32_of(&router->clients_routing));
		}
		...
		while (od_atomic_u32_of(&router->clients_routing) >=
			...
			if (!warning_emitted) {
				...
				od_error(&instance->logger,
					 ...
					 "client is waiting in routing queue, clients_routing: %u", od_atomic_u32_of(&router->clients_routing));
...
static inline int od_system_listen(od_system_t *system)
{
	...
	od_setproctitlef(
		...
		"xproxy: version %s listening and accepting new connections, configuration file: %s",
		OD_VERSION_NUMBER, instance->config_file);
...
/* 
 * Router contains rules, which contains two data structures related to
 * configs, strorage and a list of rules. During reload, strorages is cleaned through
 * od_rules_cleanup(). The list of rules are merged with new rules from the config file.
 */
void od_system_config_reload(od_system_t *system)
{
	...
	/* od_rules_validate not only validates rules, but also copies storage  to rule->storage */
	...
	rc = od_rules_storages_watchdogs_run(&instance->logger, &rules);
	if (rc == -1) {
		od_error(&instance->logger, "rules", NULL, NULL,
				 "failed to dispatch storage watchdogs");
		od_config_free(&config);
		od_rules_free(&rules);
		return;
	}
	...
	/* When reaching here, we can ensure that new configurations can be reloaded sucessfully*/
	/* So we can safely modify the original data structures like router->rules and instance->config */
	 {
		pthread_mutex_lock(&router->rules.mu);
		/* Cleanup storages. Also soft-exits watchdog. This doesn't cleanup 'rule's */
		od_rules_cleanup(&router->rules);
		od_list_move_to_tail(&router->rules.storages, &rules.storages);
		pthread_mutex_unlock(&router->rules.mu);
		od_config_reload(&instance->config, &config);
	 }

	/* All changes are done. Free unused rules */
