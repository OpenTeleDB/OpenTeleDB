...
int od_scram_parse_verifier_safe_api(od_scram_state_t *scram_state, char *verifier)
{
	char *value = NULL;
	char *scheme = NULL;
	char *iterations_raw = NULL;
	...
	char *stored_key = NULL;
	char *server_key = NULL;
	char *saveptr = NULL;  // For strtok_r state

	value = strdup(verifier);
	if (value == NULL)
		return -1;

	scheme = strtok_r(value, "$", &saveptr);
	if (scheme == NULL)
		goto error;

	iterations_raw = strtok_r(NULL, ":", &saveptr);
	if (iterations_raw == NULL)
		goto error;

	salt_raw = strtok_r(NULL, "$", &saveptr);
	if (salt_raw == NULL)
		goto error;

	stored_key_raw = strtok_r(NULL, ":", &saveptr);
	if (stored_key_raw == NULL)
		goto error;

	server_key_raw = strtok_r(NULL, "", &saveptr);
	if (server_key_raw == NULL)
		goto error;
...
static int calculate_client_proof(od_scram_state_t *scram_state,
				  const char *password, uint8_t *client_key, const char *salt,
				  int iterations,
				  const char *client_final_message,
				  uint8_t *client_proof)
{
	uint8_t client_key_buffer[OD_SCRAM_MAX_KEY_LEN];
	char *prepared_password = NULL;
	const char *errstr = NULL;
	assert(errstr == NULL);

	assert(client_key || password);

	if (client_key == NULL) {
		client_key = client_key_buffer;
...
od_scram_create_client_final_message(od_scram_state_t *scram_state,
				     char *password, uint8_t *client_key, char *auth_data,
...
#define SCRAM_FINAL_MAX_SIZE 512
	...
	rc = calculate_client_proof(scram_state, password, client_key, salt, iterations,
				    result, client_proof);
	free(salt);
	if (rc == -1)
		goto error;
					      ...
					      size_t auth_data_size)
{
	...
	/* Todo: verify server signature without salted password  */
	if (scram_state->salted_password == NULL)
		return OK_RESPONSE;
				       ...
				       char *auth_data, size_t auth_data_size)
{
	...
	client_first_message = malloc(auth_data_size + 1);
...
od_retcode_t od_scram_verify_client_proof(od_scram_state_t *scram_state,
					  char *client_proof, uint8_t *client_key_out, const char **perr)
{
	uint8_t client_signature[OD_SCRAM_MAX_KEY_LEN];
	...
	if (client_key_out)
		memcpy(client_key_out, client_key, OD_SCRAM_MAX_KEY_LEN);
...
od_retcode_t od_scram_verify_client_proof_debug(od_scram_state_t *scram_state,
					  char *client_proof, uint8_t *client_key_out, const char **perr, od_logger_t *logger,
					  const char *query_password, int query_password_len,
					  const char *auth_data, size_t auth_data_size)
{
	uint8_t client_signature[OD_SCRAM_MAX_KEY_LEN];
	uint8_t client_key[OD_SCRAM_MAX_KEY_LEN];
	uint8_t client_stored_key[OD_SCRAM_MAX_KEY_LEN];
	...
	if (memcmp(client_stored_key, scram_state->stored_key,
		   OD_SCRAM_MAX_KEY_LEN) != 0) {
        char client_stored_key_str[OD_SCRAM_MAX_KEY_LEN * 2 + 1];
        char client_key_str[OD_SCRAM_MAX_KEY_LEN * 2 + 1];
        char client_signature_str[OD_SCRAM_MAX_KEY_LEN * 2 + 1];
        char scram_state_stored_key_str[OD_SCRAM_MAX_KEY_LEN * 2 + 1];
        char server_key_str[OD_SCRAM_MAX_KEY_LEN * 2 + 1];

        for (int i = 0; i < OD_SCRAM_MAX_KEY_LEN; i++) {
            sprintf(client_stored_key_str + i * 2, "%02x", client_stored_key[i]);
            sprintf(client_key_str + i * 2, "%02x", client_key[i]);
            sprintf(client_signature_str + i * 2, "%02x", client_signature[i]);
            sprintf(scram_state_stored_key_str + i * 2, "%02x", scram_state->stored_key[i]);
            sprintf(server_key_str + i * 2, "%02x", scram_state->server_key[i]);
        }
		
		od_log(logger, "auth", NULL, NULL, "=== SCRAM STATE DUMP ===");
		od_log(logger, "auth", NULL, NULL, "iterations: %d", scram_state->iterations);
		od_log(logger, "auth", NULL, NULL, "salt: %s", scram_state->salt ? scram_state->salt : "NULL");
		od_log(logger, "auth", NULL, NULL, "client_nonce: %s", scram_state->client_nonce ? scram_state->client_nonce : "NULL");
		od_log(logger, "auth", NULL, NULL, "server_nonce: %s", scram_state->server_nonce ? scram_state->server_nonce : "NULL");
		od_log(logger, "auth", NULL, NULL, "client_first_message: %s", scram_state->client_first_message ? scram_state->client_first_message : "NULL");
		od_log(logger, "auth", NULL, NULL, "server_first_message: %s", scram_state->server_first_message ? scram_state->server_first_message : "NULL");
		od_log(logger, "auth", NULL, NULL, "client_final_message: %s", scram_state->client_final_message ? scram_state->client_final_message : "NULL");
		od_log(logger, "auth", NULL, NULL, "salted_password: %s", scram_state->salted_password ? "NOT NULL" : "NULL");
		od_log(logger, "auth", NULL, NULL, "stored_key: %s", scram_state_stored_key_str);
		od_log(logger, "auth", NULL, NULL, "server_key: %s", server_key_str);
		od_log(logger, "auth", NULL, NULL, "=== INPUT PARAMETERS ===");
		if (query_password)
			od_log(logger, "auth", NULL, NULL, "query_password: %.*s (len: %d)", query_password_len, query_password, query_password_len);
		else
			od_log(logger, "auth", NULL, NULL, "query_password: NULL");
		if (auth_data)
			od_log(logger, "auth", NULL, NULL, "auth_data: %.*s (size: %zu)", (int)auth_data_size, auth_data, auth_data_size);
		else
			od_log(logger, "auth", NULL, NULL, "auth_data: NULL");

		od_log(logger, "auth", NULL, NULL, "=== COMPUTED VALUES ===");
		od_log(logger, "auth", NULL, NULL, "client_stored_key: %s", client_stored_key_str);
		od_log(logger, "auth", NULL, NULL, "client_key: %s", client_key_str);
		od_log(logger, "auth", NULL, NULL, "client_signature: %s", client_signature_str);
		od_log(logger, "auth", NULL, NULL, "=== END SCRAM STATE DUMP ===");
