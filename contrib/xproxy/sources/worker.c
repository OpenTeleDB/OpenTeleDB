...
static inline void od_worker(void *arg)
{
	...
	for (;;) {
		machine_msg_t *msg;
		int data_available_on_timeout = 0;
		void *timedout_msg_ptr = NULL;
		int status = -1;
		int  receive_data_on_timeout = 1;

		/* Inverse priorities of cliend routing to decrease chances of timeout */
		msg = machine_channel_read_back(worker->task_channel, 1000, &data_available_on_timeout, &timedout_msg_ptr, &status, receive_data_on_timeout);
		if (data_available_on_timeout) {
			od_log(&instance->logger,"worker", NULL, NULL, "reader timeout but msg is available, msg: %p, status: %d", timedout_msg_ptr, status) ;
		}
		if (msg == NULL) {
			if (worker->shutdown)
				break;
		...
		case OD_MSG_CLIENT_NEW: {
			...
			if (coroutine_id == -1) {
				...
				od_error(&instance->logger, "worker", client,NULL, "failed to create coroutine, decreased, clients_routing: %u",od_atomic_u32_of(&router->clients_routing));
				od_client_free(client);
				break;
			}
			od_log(&instance->logger,"worker", client, NULL, "client coroutine created, clients_routing: %u", od_atomic_u32_of(&router->clients_routing));
