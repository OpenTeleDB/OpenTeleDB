					     ...
					     od_rules_t *rules)
{
	...
	od_list_foreach(&rules->storages, i)
	{
		...
		if (storage->watchdog) {
			...
			rc = od_storage_watchdog_launch(storage);
			if (rc != OK_RESPONSE) {
				od_error(logger, "system", NULL, NULL,
					 "failed to launch storage watchdogs");
				return rc;
			}
		}
	}
...
void od_rules_group_checker_run(void *arg)
{
	...
	for (;;) {
		...
		status = od_router_attach(router, group_checker_client, REPLICA_ROLE_ANY, false);
		od_debug(
...
/* Unref and free if there's not refs left */
...
bool od_storage_endpoints_compare_same(const od_storage_endpoint_t *a,  
						const od_storage_endpoint_t *b) 
{
	if (a->host && b->host) {
...
bool od_storage_endpoints_same_host_and_port(const od_storage_endpoint_t *a,  
						const od_storage_endpoint_t *b) 
{
	if (a->host && b->host) {
		if (strcmp(a->host, b->host) != 0) 
					   ...
					   od_rule_storage_t *b)
{
	...
	/* Given that endpoints_counts are the same, check for od_storage_endpoint_t */
	for (size_t i = 0; i < a->endpoints_count; ++i)
	{
		if (!od_storage_endpoints_compare_same(&a->endpoints[i], &b->endpoints[i]))
			return 0;
	}
...
int od_rules_rule_compare(od_rule_t *a, od_rule_t *b)
{
	...
	if (a->enable_quantiles_stat != b->enable_quantiles_stat)
		return 0;
	...
	if (a->replication_delay_threshold != b->replication_delay_threshold) {
		return 0;
	}
	...
	if (a->enable_read_only_blacklist != b->enable_read_only_blacklist) {
		return 0;
	}
	...
	if (a->quantiles && b->quantiles) {
		for (int i = 0; i < a->quantiles_count; ++i) {
					...
					od_list_t *to_drop)
{
	...
	od_list_foreach_safe(&src->rules, i, n)
	{
		...
		/* 
		 *	If origin rule exists (successfully matched), do:
		 *	1. completely the same, do nothing
		 *	2. different, but not needed to disconnect, do nothing
		 *	3. different, and needed to disconnect, drop origin rule
		*/
		if (origin) {
			/* if the origin rule is the same as new rule */
		     ...
		     char *user_name, od_address_range_t *address_range)
{
	...
	// always reserve prepared statement for transaction pool
	if (pool->pool == OD_RULE_POOL_TRANSACTION) {
		if (!pool->reserve_prepared_statement) {
			pool->reserve_prepared_statement = true;
			od_log(
				logger, "rules", NULL, NULL,
				"rule '%s.%s %s': always reserve prepared statements in transaction pool",
				db_name, user_name, address_range->string_value);
		}
	}
	// never reserve prepared statement for session pool
	else if (pool->reserve_prepared_statement) {
		od_log(
			logger, "rules", NULL, NULL,
			"rule '%s.%s %s': reserve prepared statements support in session pool makes no sence",
			db_name, user_name, address_range->string_value);
		pool->reserve_prepared_statement = false;
	}
		      ...
		      od_logger_t *logger)
{
	...
	od_list_foreach(&rules->storages, i)
	{
		...
		if (storage->storage_type == OD_RULE_STORAGE_REMOTE)
		{
			if (storage->endpoints == NULL) {
				od_error(logger, "rules", NULL, NULL, "storage endpoints requeird");
				return -1;
			}
			for (size_t i = 0; i < storage->endpoints_count; ++i)
			{
				if (storage->endpoints[i].host == NULL){
					od_error(logger, "rules", NULL, NULL, "storage %s endpoint.host requeird", storage->name);
					return -1;
				}
				if (storage->endpoints[i].port == 0){
					od_error(logger, "rules", NULL, NULL, "storage %s endpoint.port requeird", storage->name);
					return -1;
				}
				if (storage->endpoints[i].application_name == NULL){
					od_error(logger, "rules", NULL, NULL, "storage %s endpoint.application_name requeird", storage->name);
					return -1;
				}
			}
			...
			if (total_weight == 0.0) {
				od_error(logger, "rules", NULL, NULL, "storage %s accumulation of endpoint weights should not be 0.0", storage->name);
				return -1;
			}
		}
	}
	...
	od_list_foreach(&rules->rules, i)
	{
		...
		/* auth */
		if (!rule->auth) {
			/* donot check auth if rule used by watchdog */
			if (strcasecmp(rule->db_name, "watchdog_int")) {
				od_error(
					logger, "rules", NULL, NULL,
		...
		if (strcmp(rule->auth, "none") == 0) {
			...
			if (rule->password == NULL
			   ...
			   && rule->allow_clear_text_frontend_auth == false
#ifdef PAM_FOUND
			    && rule->auth_pam_service == NULL
#endif
		...
		} else if (strcmp(rule->auth, "md5_or_scram") == 0) {
			rule->auth_mode = OD_RULE_AUTH_MD5_OR_SCRAM;
			if (rule->auth_query == NULL) {
				od_error(logger, "rules", NULL, NULL,
					 "rule '%s.%s %s': auth_query is not set",
					 rule->db_name, rule->user_name,
					 rule->address_range.string_value);
				return -1;
			}
...
void od_rules_print(od_rules_t *rules, od_logger_t *logger)
{
	...
	od_list_foreach(&rules->rules, i)
	{
		...
		od_log(logger, "rules", NULL, NULL,
		       "  pool idle_timeout_whitelist %s",
		       rule->pool->idle_timeout_whitelist);
		od_log(logger, "rules", NULL, NULL,
		...
		od_log(logger, "rules", NULL, NULL,
		       "  enable_read_only_blacklist                  %s",
		       od_rules_yes_no(rule->enable_read_only_blacklist));
#ifdef LDAP_FOUND
		if (rule->ldap_endpoint_name) {
			od_log(logger, "rules", NULL, NULL,
		...
		if (rule->replication_delay_threshold)
			od_log(logger, "rules", NULL, NULL,
			       "  replication delay threshold   %ld", rule->replication_delay_threshold);
