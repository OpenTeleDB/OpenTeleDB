...
extern void od_logger_write(od_logger_t *, od_logger_level_t, char *, int , char *, void *,
			    void *, char *, va_list);
extern void od_logger_write_plain(od_logger_t *, od_logger_level_t, char *,
				  void *, void *, char *);

static inline void od_log_helper(od_logger_t *logger, od_logger_level_t level, char *file, int line, char *context, void *client,
			  void *server, char *fmt, ...)
{
	va_list args;
	va_start(args, fmt);
	od_logger_write(logger, level, file, line, context, client, server, fmt, args);
	va_end(args);
}
...
#define od_log(logger, ...) \
        od_log_helper(logger, OD_LOG, __FILE_NAME__, __LINE__, __VA_ARGS__)

#define od_debug(logger, ...) \
        od_log_helper(logger, OD_DEBUG, __FILE_NAME__, __LINE__, __VA_ARGS__)

#define od_error(logger, ...) \
        od_log_helper(logger, OD_ERROR, __FILE_NAME__, __LINE__, __VA_ARGS__)

#define od_fatal(logger, ...) \
        od_log_helper(logger, OD_FATAL, __FILE_NAME__, __LINE__, __VA_ARGS__)
