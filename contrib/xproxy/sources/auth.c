...
static inline int od_auth_frontend_cleartext_pass(od_client_t *client)
{
	...
	if (current_time - cache_value->timestamp < 10 * interval_usec) {
		...
		if (cache_value->passwd_len == (uint32_t)password->password_len &&
			memcmp(cache_value->passwd, password->password,
	...
	/* Try create a new connection to backend pg to do authentication */
	od_router_status_t status = od_router_attach_new(client, "auth");
	if (client->server) {
	...
	if (status == OD_ROUTER_OK) {
		...
		if (cache_value->passwd) {
			memcpy(cache_value->passwd, password->password, password->password_len);
...
exit:
	od_hashmap_unlock_key(storage->bcache, keyhash, &key);
	return res;
}
...
static inline int od_auth_frontend_cleartext(od_client_t *client)
{
	...
	if (route->rule->enable_password_passthrough ||
		route->rule->allow_clear_text_frontend_auth) {
	...
	/* */
	if (client->rule->allow_clear_text_frontend_auth) {
		return od_auth_frontend_cleartext_pass(client);
	}
...
static inline int od_auth_frontend_scram_sha_256(od_client_t *client)
{
	...
	rc = od_scram_parse_verifier_safe_api(&scram_state, query_password.password);
	if (rc == -1)
	...
	const char *client_proof_err = NULL;
#if OD_DEBUG_SCRAM == -1
	rc = od_scram_verify_client_proof(&scram_state, client_proof, client->scram_client_key, &client_proof_err);
#else 
	rc = od_scram_verify_client_proof_debug(&scram_state, client_proof, client->scram_client_key, &client_proof_err, &instance->logger,
		query_password.password, query_password.password_len, auth_data, auth_data_size);
#endif
	...
	if (rc == -1) {
		od_frontend_error(
			client, KIWI_INVALID_AUTHORIZATION_SPECIFICATION,
			"frontend auth: password authentication failed, err:%s", client_proof_err ? client_proof_err : "NULL");
...
static inline int od_auth_frontend_md5_or_scram(od_client_t *client)
{
	od_instance_t *instance = client->global->instance;
	char peer[128];

	if (client->rule->auth_query == NULL) {
		od_error(&instance->logger, "auth", client, NULL,
				"rule '%s.%s' need specify query_query",
				client->rule->db_name, client->rule->user_name);
		return -1;
	}

	od_getpeername(client->io.io, peer, sizeof(peer), 1, 0);
	int rc = od_auth_query(client, peer);
	if (rc == -1) {
		od_error(&instance->logger, "auth", client, NULL,
				"failed to make auth_query");
		od_frontend_error(
	...
	if (strncasecmp(password->password, "md5", 3) == 0) {
		return od_auth_frontend_md5(client);
	}
#ifdef USE_SCRAM
	else if(strncasecmp(password->password, "scram-sha-256", 13) == 0) {
		return od_auth_frontend_scram_sha_256(client);
	}
#endif
	else {
		int len = password->password_len;
		if (len > 8)
			len = 8;

		od_error(&instance->logger, "auth", client, NULL,
				"unkown password_encryption: %.*s*******", len, password->password);
		return -1;
	}
}
...
{
	od_instance_t *instance = client->global->instance;
	od_rule_auth_type_t auth_mode = client->rule->auth_mode;
	int rc;

	if (client->rule->allow_clear_text_frontend_auth) {
		auth_mode = OD_RULE_AUTH_CLEAR_TEXT;
	}
	...
	case OD_RULE_AUTH_NONE:
		break;
	case OD_RULE_AUTH_MD5_OR_SCRAM:
		rc = od_auth_frontend_md5_or_scram(client);
		if (rc == -1)
			return -1;
		break;
	default:
				      ...
				      od_client_t *client)
{
	...
	if (route->rule->storage_user) {
		...
		user = client->startup.user.value;
		user_len = client->startup.user.value_len - 1;
	}
						...
						od_client_t *client)
{
	...
	if (route->rule->storage_password) {
		...
		client_key = client->scram_client_key;
	} else if (route->rule->password) {
		password = route->rule->password;
	...
	machine_msg_t *msg = od_scram_create_client_final_message(
		&server->scram_state, password, client_key, auth_data, auth_data_size);
	if (msg == NULL) {
		od_error(&instance->logger, "auth", NULL, server,
