			   ...
			   void **argv)
{
	...
	info.server_pool_active = od_server_pool_active(route->server_pool, route->server_pool_size);
	info.server_pool_idle = od_server_pool_idle(route->server_pool, route->server_pool_size);
...
static inline void od_cron_expire(od_cron_t *cron)
{
	...
	od_list_t expire_list, prep_stmt_clean_list;
	od_list_init(&expire_list);
	od_list_init(&prep_stmt_clean_list);

	int rc;
	rc = od_router_expire(router, &expire_list, &prep_stmt_clean_list);
	if (rc > 0) {
		od_list_t *i, *n;
		...
		od_list_foreach_safe(&prep_stmt_clean_list, i, n)
		{
			od_server_t *server;
			server = od_container_of(i, od_server_t, link);
			od_debug(&instance->logger, "clean", NULL, server,
					"cleaning prepared statements in server connection (%d secs)",
					server->idle_time);
			if (cron_close_expired_prep_stmt(server) != OD_OK) {
				server->route = NULL;
				od_backend_close_connection(server);
				od_backend_close(server);
			}
