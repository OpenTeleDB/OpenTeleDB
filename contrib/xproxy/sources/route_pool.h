...
{
	assert(rule->storage);
	int server_count = rule->storage->endpoints_count;
	od_route_t *route = od_route_allocate(server_count);
	if (route == NULL)
		return NULL;
	int rc;
	...
	route->rule = rule;
	if (rule->enable_quantiles_stat) {
		route->stats.enable_quantiles = true;
		route->stats.transaction_hgram = od_histogram_new(400, 0, 1200000000); // 20min
		route->stats.query_hgram = od_histogram_new(400, 0, 1200000000);
		route->stats.pool_wait_hgram = od_histogram_new(400, 0, 60000000); // 1min
	}
				      ...
				      void **argv)
{
	...
	od_list_foreach(&pool->list, i)
	{
		...
		od_stat_t current;
		od_stat_init(&current, route->server_pool_size);
		od_stat_copy(&current, &route->stats);

		/* calculate average */
		od_stat_t avg;
		od_stat_init(&avg, route->server_pool_size);
		if (route->stats.enable_quantiles) {
			od_histogram_get_pct_checkpoint(route->stats.transaction_hgram, 0);
			od_histogram_get_pct_checkpoint(route->stats.query_hgram, 0);
			od_histogram_get_pct_checkpoint(route->stats.pool_wait_hgram, 0);
		}
			    ...
			    uint64_t prev_time_us, void **argv)
{
	...
	od_list_foreach(&pool->list, i)
	{
		...
		od_stat_t prev;
		od_stat_init(&current, route->server_pool_size);
		od_stat_init(&prev, route->server_pool_size);
