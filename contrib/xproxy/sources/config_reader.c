...
typedef enum {
	...
	OD_LUSE_UNIX_SOCKET_IF_POSSIBLE,
	OD_LLOCKS_DIR,
	...
	OD_LSERVER_LIFETIME,
	OD_LENABLE_READ_ONLY_BLACKLIST,
	OD_LTLS,
	OD_LTLS_CA_FILE,
	OD_LTLS_KEY_FILE,
	OD_LTLS_CERT_FILE,
	...
	OD_LPOOL_PREPARED_STATEMENT_LIMIT,
	OD_LPOOL_PREPARED_STATEMENT_EXPIRED_TIME,
	OD_LPOOL_TARGET_SERVER_ATTRS,
	OD_LPOOL_CLIENT_IDLE_TIMEOUT,
	OD_LPOOL_IDLE_TIMEOUT_WHITELIST,
	OD_LPOOL_IDLE_IN_TRANSACTION_TIMEOUT,
	OD_LSTORAGE_DB,
	OD_LSTORAGE_USER,
	OD_LSTORAGE_PASSWORD,
	OD_LAUTHENTICATION,
	OD_LALLOW_CLEAR_TEXT_FRONTEND_AUTH,
	...
	OD_LPRIMARY_ARBITRATION,
	OD_LWATCHDOG_LAG_INTERVAL,
	OD_LCATCHUP_TIMEOUT,
	OD_LCATCHUP_CHECKS,
	OD_LREPLICATION_DELAY_THRESHOLD,
	OD_LOPTIONS,
...
static od_keyword_t od_config_keywords[] = {
	...
	od_keyword("use_unix_socket_if_possible", OD_LUSE_UNIX_SOCKET_IF_POSSIBLE),
	...
	od_keyword("port_attrs", OD_LPORTATTRS),
	...
	od_keyword("enable_read_only_blacklist", OD_LENABLE_READ_ONLY_BLACKLIST),
	...
	od_keyword("endpoints", OD_LENDPOINTS),
	od_keyword("endpoint", OD_LENDPOINT),
	od_keyword("hostname", OD_LHOSTNAME),
	od_keyword("weight", OD_LWEIGHT),
	od_keyword("application_name", OD_LAPPLICATION_NAME),
	od_keyword("node_id", OD_LNODE_ID),
	...
	od_keyword("pool_reserve_prepared_statement",
		   OD_LPOOL_RESERVE_PREPARED_STATEMENT),
	od_keyword("pool_prepared_statement_limit",
		   OD_LPOOL_PREPARED_STATEMENT_LIMIT),
	od_keyword("pool_prepared_statement_expired_time",
		   OD_LPOOL_PREPARED_STATEMENT_EXPIRED_TIME),
	od_keyword("target_server_attrs",
			   OD_LPOOL_TARGET_SERVER_ATTRS),
	od_keyword("pool_client_idle_timeout", OD_LPOOL_CLIENT_IDLE_TIMEOUT),
	od_keyword("pool_idle_timeout_whitelist",
		   OD_LPOOL_IDLE_TIMEOUT_WHITELIST),
	...
	od_keyword("allow_clear_text_frontend_auth", OD_LALLOW_CLEAR_TEXT_FRONTEND_AUTH),
	...
	od_keyword("primary_arbitration", OD_LPRIMARY_ARBITRATION),
	...
	od_keyword("replication_delay_threshold", OD_LREPLICATION_DELAY_THRESHOLD),
	...
	od_keyword("enable_quantiles_state", OD_LENABLE_QUANTILES),
	{ 0, 0, 0 },
};
...
/*
 * Reads the list string from the config file, and splits it into individual items.
 */
static bool od_config_reader_string_list(od_config_reader_t *reader, char ***list, size_t *list_cnt)
{
	size_t i;
	size_t count;
	size_t len;
	...
	if (!od_config_reader_string(reader, &list_string)) {
		return false;
	}
	...
	*list = (char **)malloc(count * sizeof(char *));
	...
	/* Split the list string into individual items. */
	for (i = 0; i < len;) {

		while (isspace(list_string[i]) || list_string[i] == ','){
			i++;
		}
		...
		if (i < len) {
			...
			while (i < len && !isspace(list_string[i]) && list_string[i] != ',') {
				i++;
			}

			item_len = i - item_start;

			(*list)[count] = (char *)malloc(item_len + 1);
			if ((*list)[count] == NULL) {
				return false;
			}
			memcpy((*list)[count], list_string + item_start, item_len);
			((*list)[count])[item_len] = '\0';
			count++;
		}
	}
...
static bool od_config_reader_number_bigint(od_config_reader_t *reader, int64_t *number)
{
	od_token_t token;
	int rc;
	rc = od_parser_next(&reader->parser, &token);
...
static bool od_config_reader_number_float(od_config_reader_t *reader, double *number)
{
	od_token_t token;
	int rc;
	rc = od_parser_next(&reader->parser, &token);
	if (rc != OD_PARSER_INT && rc != OD_PARSER_FLOAT) {
		od_parser_push(&reader->parser, &token);
		od_config_reader_error(reader, &token, "expected 'number'");
		return false;
	}
	*number = token.type == OD_PARSER_FLOAT ? token.value.float_num : (double) token.value.num;
	return true;
}
...
static int od_config_reader_storage_endpoint(od_config_reader_t *reader,
					 od_rule_storage_t *storage, size_t idx)
{
	/* Default node_id is 0, which is suitable for cenrealized DBMS */
	storage->endpoints[idx].node_id = 0;
	...
	for (;;){
		...
		/* ip */
		case OD_LHOSTNAME:
			if (!od_config_reader_string(reader, &storage->endpoints[idx].host))
				goto error;
			continue;
		/* port */
		case OD_LPORT:
			if (!od_config_reader_number(reader,  &storage->endpoints[idx].port))
				goto error;
			continue;
		/* weight */
		case OD_LWEIGHT:
			if (!od_config_reader_number_float(reader,  &storage->endpoints[idx].weight))
				goto error;
			continue;
		/* application name */
		case OD_LAPPLICATION_NAME:
			if (!od_config_reader_string(reader, &storage->endpoints[idx].application_name))
				goto error;
			continue;
		/* node id */
		case OD_LNODE_ID:
			if (!od_config_reader_number(reader, &storage->endpoints[idx].node_id))
				goto error;
			continue;
		default: {
			od_config_reader_error(reader, &token,
...
static int od_config_reader_storage_host_assign(od_rule_storage_t *storage)
{
	char* host = NULL;
	size_t length = 0;
	...
	for(size_t i = 0; i < storage->endpoints_count; ++i){
		...
		int port = storage->endpoints[i].port;
		do {
			port_length++;
			port /= 10;
		} while (port != 0);

		if (storage->endpoints[i].host == NULL) 
        	return NOT_OK_RESPONSE;

		length += strlen(storage->endpoints[i].host);
		length += port_length;
	}

	/* for seperator ',' and ':' */
	length += storage->endpoints_count -1;
	length += storage->endpoints_count;
	length += 1;
    ...
    for (size_t i = 0; i < storage->endpoints_count; ++i) {
		char port_str[20];
		snprintf(port_str, sizeof(port_str), "%d", storage->endpoints[i].port);
        strcat(host, storage->endpoints[i].host);
		strcat(host, ":");
		strcat(host, port_str);
        if (i < storage->endpoints_count - 1) 
            strcat(host, ",");
	}
	storage->host = host;
    return OK_RESPONSE;
...
static int od_config_reader_storage_endpoints(od_config_reader_t *reader,
					 od_rule_storage_t *storage)
{
	...
	size_t endpoint_cnt = 0;
	storage->endpoints = malloc(sizeof(od_storage_endpoint_t));
	if (storage->endpoints == NULL) {
		return NOT_OK_RESPONSE;
	}
	memset(storage->endpoints, 0, sizeof(od_storage_endpoint_t));
	...
	for(;;){
		...
		case OD_PARSER_SYMBOL:
			/* } */
			if (token.value.num == '}') {
				storage->endpoints_count = endpoint_cnt;
				rc = od_config_reader_storage_host_assign(storage);
				if (rc == NOT_OK_RESPONSE) {
					od_config_reader_error(reader, &token, "error while assigning storage host in od_config_reader_storage_host_assign");
					goto error;
				}
				return OK_RESPONSE;
			}
		...
		if (keyword->id != OD_LENDPOINT){ 
			od_config_reader_error(reader, &token,
					       "expect endpoint, got others");
			goto error;
		}
		
		if (od_config_reader_storage_endpoint(reader, storage, endpoint_cnt) != OK_RESPONSE)
			goto error;
		endpoint_cnt++;
		storage->endpoints = realloc(storage->endpoints, sizeof(od_storage_endpoint_t) * (endpoint_cnt + 1));
		if (storage->endpoints == NULL) {
			goto error;
		}
		memset(storage->endpoints + endpoint_cnt, 0, sizeof(od_storage_endpoint_t));
	}
...
static int od_config_reader_listen(od_config_reader_t *reader)
{
	...
	for (;;) {
		...
		case OD_LPORT:
			...
			continue;
		case OD_LPORTATTRS:
			if (!od_config_reader_string(reader, &attr))
				return NOT_OK_RESPONSE;

			if (strcasecmp(attr, "read-write") == 0) {
				listen->port_attrs = OD_PORT_ATTR_RW;
			} else if (strcasecmp(attr, "read-only") == 0) {
				listen->port_attrs = OD_PORT_ATTR_RO;
			} else if (strcasecmp(attr, "write-only") == 0) {
				listen->port_attrs = OD_PORT_ATTR_WO;
			} else {
				od_config_reader_error(reader, &token,
									   "unexpected port_attrs");
				return NOT_OK_RESPONSE;
			}
			continue;
				    ...
				    od_extention_t *extentions)
{
	...
	for (;;) {
		...
		/* endpoints */
		case OD_LENDPOINTS:
			if (od_config_reader_storage_endpoints(reader, storage) !=
			    OK_RESPONSE)
				goto error;
			continue;
...
{
	char *tmp = NULL;
	char *crypted_storage_password = NULL;
	char *crypted_password = NULL;
	char decrypted_storage_password[64] = {0};
	...
	for (;;) {
...
#endif
			break;
		case OD_LALLOW_CLEAR_TEXT_FRONTEND_AUTH:
			if (!od_config_reader_yes_no(reader, &rule->allow_clear_text_frontend_auth))
				return NOT_OK_RESPONSE;
			break;
		/* auth_common_name */
		...
		/* password */
		case OD_LPASSWORD:
			if (!od_config_reader_string(reader, &crypted_password))
				return NOT_OK_RESPONSE;
			if (decrypt_string(crypted_password, 
							MY_KEY, decrypted_password) == -1) {
			...
			rule->password = strdup(decrypted_password);
		...
		case OD_LENABLE_QUANTILES: {
			if (!od_config_reader_yes_no(
					reader,
					&rule->enable_quantiles_stat)) {
				return NOT_OK_RESPONSE;
			}
		}  break;
		...
		case OD_LENABLE_READ_ONLY_BLACKLIST:
			if (!od_config_reader_yes_no(reader,
						     &rule->enable_read_only_blacklist))
				return NOT_OK_RESPONSE;
			continue;
#ifdef LDAP_FOUND
		/* ldap_pool_size */
		...
		case OD_LPOOL_RESERVE_PREPARED_STATEMENT:
			if (!od_config_reader_yes_no(
				...
				return NOT_OK_RESPONSE;
			continue;
		case OD_LPOOL_TARGET_SERVER_ATTRS:
			if (!od_config_reader_string(reader, &tmp)) {
				return NOT_OK_RESPONSE;
			}

			if (strcmp(tmp, "auto") == 0) {
				rule->pool->target_server_attrs = OD_TARGET_SERVER_AUTO;
			} else if (strcmp(tmp, "read-write") == 0) {
				rule->pool->target_server_attrs = OD_TARGET_SERVER_RW;
			} else if (strcmp(tmp, "read-only") == 0) {
				rule->pool->target_server_attrs = OD_TARGET_SERVER_RO;
			} else {
				return NOT_OK_RESPONSE;
			}
			...
			continue;
		case OD_LPOOL_PREPARED_STATEMENT_LIMIT:
			if (!od_config_reader_number(
				    reader,
				    &rule->pool->prepared_statement_limit))
				return NOT_OK_RESPONSE;
			continue;
		case OD_LPOOL_PREPARED_STATEMENT_EXPIRED_TIME:
			if (!od_config_reader_number(
				    reader,
				    &rule->pool->prepared_statement_expired_time))
				return NOT_OK_RESPONSE;
			continue;
		...
		case OD_LPOOL_CLIENT_IDLE_TIMEOUT:
			...
			continue;
		/* pool_idle_timeout_whitelist */
		case OD_LPOOL_IDLE_TIMEOUT_WHITELIST:
			if (!od_config_reader_string_list(
				    reader, &rule->pool->idle_timeout_whitelist,
					&rule->pool->idle_timeout_whitelist_count)) {
				return NOT_OK_RESPONSE;
			}
			continue;
		...
		case OD_LSTORAGE_PASSWORD:
			if (!od_config_reader_string(reader,
						     &crypted_storage_password))
				return NOT_OK_RESPONSE;
			if (decrypt_string(crypted_storage_password, 
							MY_KEY,decrypted_storage_password) == -1) {
				free(crypted_storage_password);
				return NOT_OK_RESPONSE;
			}
			if (rule->storage_password) {
				free(rule->storage_password);
			}
			rule->storage_password = strdup(decrypted_storage_password);
			rule->storage_password_len = strlen(rule->storage_password);
			free(crypted_storage_password);
			crypted_storage_password = NULL;
			memset(decrypted_storage_password, 0, sizeof(decrypted_storage_password));
			continue;
		/* log_debug */
		case OD_LLOG_DEBUG:
		...
		case OD_LPRIMARY_ARBITRATION:
			...
			if (!od_config_reader_string(reader,
						     &watchdog->primary_arbitration_cmd)) {
				return NOT_OK_RESPONSE;
			}
			continue;
		...
		case OD_LCATCHUP_CHECKS:
			...
			continue;
		case OD_LREPLICATION_DELAY_THRESHOLD:
			if (!od_config_reader_number_bigint(reader,
						     &rule->replication_delay_threshold)) {
				return NOT_OK_RESPONSE;
			}
			continue;
		/* options */
				  ...
				  od_extention_t *extentions)
{
	...
	for (;;) {
		...
		case OD_LUNIX_SOCKET_MODE:
			...
			continue;
		/* use_unix_socket_if_possible*/
		case OD_LUSE_UNIX_SOCKET_IF_POSSIBLE:
			if (!od_config_reader_yes_no(
				    reader, &config->use_unix_socket_if_possible)) {
				goto error;
			}
			continue;
		/* locks_dir */
		...
		case OD_LLOG_FORMAT:
			...
			size_t len = strlen(config->log_format);
			if (len == 0 || config->log_format[len - 1] != 'n' ) {
				char *new_log_format = realloc(config->log_format, len + 3);
				if (new_log_format == NULL) {
					free(config->log_format);
					goto error;
				}
				config->log_format = new_log_format;
				config->log_format[len] = '\\';
				config->log_format[len + 1] = 'n';
				config->log_format[len + 2] = '\0';
			}
			continue;
		/* log_file */
		case OD_LLOG_FILE:
