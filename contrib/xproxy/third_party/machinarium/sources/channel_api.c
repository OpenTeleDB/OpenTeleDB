...
MACHINE_API machine_msg_t *machine_channel_read_back(machine_channel_t *obj,
						     uint32_t time_ms, int* data_available_on_timeout, void** timedout_msg_ptr, int *status, int receive_data_on_timeout)
{
	mm_channeltype_t *type;
	type = mm_cast(mm_channeltype_t *, obj);
	if (type->is_shared) {
		...
		channel = mm_cast(mm_channel_t *, obj);
		mm_msg_t *msg;
		msg = mm_channel_read_back(channel, time_ms, data_available_on_timeout, timedout_msg_ptr, status, receive_data_on_timeout);
		return (machine_msg_t *)msg;
	}
	mm_channelfast_t *channel;
	channel = mm_cast(mm_channelfast_t *, obj);
