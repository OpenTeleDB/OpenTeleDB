...
mm_msg_t *mm_channel_read_back(mm_channel_t *channel, uint32_t time_ms, int *data_available_on_timeout, void **timedout_msg_ptr, int *status, int receive_data_on_timeout)
{
	/* try to get first message, if no other readers are
	 * waiting, otherwise put reader in the wait
	 * channel */
	...
	if (reader.event.call.status != 0) {
		if (reader.result) {
			/* 向调用者反馈：超时唤醒但数据已经到达 */
			*data_available_on_timeout = 1;
			*timedout_msg_ptr = reader.result;
			*status = reader.event.call.status;
			/* 仍然返回数据, 或丢弃数据 */
			if (receive_data_on_timeout == 1) 
				return reader.result;
			else 
				mm_msg_unref(&mm_self->msg_cache, reader.result);
		}
