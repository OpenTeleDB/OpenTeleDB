...
KIWI_API static inline int
kiwi_be_read_notice_message(char *data, uint32_t size, char **message, size_t *message_len)
{
	kiwi_header_t *header = (kiwi_header_t *)data;
	uint32_t len;
	int rc = kiwi_read(&len, &data, &size);
	if (kiwi_unlikely(rc != 0))
		return -1;
	if (kiwi_unlikely(header->type != KIWI_BE_NOTICE_RESPONSE))
		return -1;
	...
	*message = NULL;
	*message_len = 0;
	while (len > 0 && *message == NULL) {
		if (kiwi_unlikely(kiwi_read8(&type, &pos, &len) != 0))
			return -1;
		if (type == 'M') 
			*message = pos;
		if (kiwi_readsz(&pos, &len) != 0)
			return -1;
	}

	if (*message)
		*message_len = pos - *message;
