...
typedef enum {
	...
	KIWI_VAR_SCRAM_ITERATIONS,
	KIWI_VAR_SESSION_AUTHORIZATION,
	KIWI_VAR_DATESTYLE,
	KIWI_VAR_INTERVALSTYLE,
	KIWI_VAR_TIMEZONE,
...
{
	kiwi_var_init(&vars->vars[KIWI_VAR_SCRAM_ITERATIONS], "scram_iterations",
			  sizeof("scram_iterations"));
	kiwi_var_init(&vars->vars[KIWI_VAR_SESSION_AUTHORIZATION], "session_authorization",
	...
	kiwi_var_init(&vars->vars[KIWI_VAR_INTERVALSTYLE], "intervalStyle", 14);
...
__attribute__((hot)) static inline int kiwi_vars_cas(kiwi_vars_t *client,
						     kiwi_vars_t *server,
							 kiwi_vars_t *defaults,
						     char *query, int query_len)
{
	int pos = 0;
	kiwi_var_type_t type;
	type = KIWI_VAR_CLIENT_ENCODING;
	for (; type < KIWI_VAR_TRANSACTION_ISOLATION; type++) {
		kiwi_var_t *var;
		...
		// 	continue;
		if (var->type == KIWI_VAR_UNDEF && server_var->type == KIWI_VAR_UNDEF)
			continue;

		bool use_default = false;
		if (var->type == KIWI_VAR_UNDEF) {
			var = kiwi_vars_of(defaults, type);
			use_default = true;
		}

		if (kiwi_var_compare(var, server_var) == 0) {
			...
			/* set server_var's type to KIWI_VAR_UNDEF*/
			if (use_default) {
				server_var->type = KIWI_VAR_UNDEF;
			}
		}
	}
